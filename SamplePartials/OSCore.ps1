...
{ 
    ...
    Node $TargetName
    {
        ...
        foreach($network in $networks)
        {
            ...
            if ($adapterMAC)
            {
                $targetAdapter = Invoke-Command -ComputerName $TargetName -ScriptBlock { Get-NetAdapter | Where-Object {$_.MacAddress -eq $using:adapterMAC}} -Credential $domainCred -ErrorAction SilentlyContinue
                $uid = $adapterMAC
            }
            else
            {
                $targetAdapter = Invoke-Command -ComputerName $TargetName -ScriptBlock { Get-NetIPInterface -InterfaceAlias $using:adapterAlias -AddressFamily $using:addressFamily } -Credential $domainCred -ErrorAction SilentlyContinue
                $uid = $adapterAlias
            }
